...
async function loadAdvertisedTenders() {
  ...
  try {
    const { data, error } = await supabase
      .from("advertised_tenders") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No advertised tenders available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: advertised_tenders
          const { data: signedData, error: signedError } = await supabase.storage
            .from("advertised_tenders")
    ...
    spinner.style.display = "none";
    console.error("Error fetching advertised tenders:", err);
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load advertised tenders.";
  }
}

document.addEventListener("DOMContentLoaded", loadAdvertisedTenders);
