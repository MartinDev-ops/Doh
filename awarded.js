...
async function loadAwardedTenders() {
  ...
  try {
    const { data, error } = await supabase
      .from("awarded_tenders") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No awarded tenders available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: awarded_tenders
          const { data: signedData, error: signedError } = await supabase.storage
            .from("awarded_tenders")
    ...
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load awarded tenders.";
  }
}

document.addEventListener("DOMContentLoaded", loadAwardedTenders);
