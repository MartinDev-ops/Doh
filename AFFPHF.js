...
async function loadApplicationForms() {
  ...
  try {
    const { data, error } = await supabase
      .from("application_forms_for_private_health_facilities") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No application forms available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: application_forms_for_private_health_facilities
          const { data: signedData, error: signedError } = await supabase.storage
            .from("application_forms_for_private_health_facilities")
    ...
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load application forms.";
  }
}

document.addEventListener("DOMContentLoaded", loadApplicationForms);
