...
async function loadStrategicPlans() {
  ...
  try {
    const { data, error } = await supabase
      .from("department_strategic_plan") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No strategic plans available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: department_strategic_plan
          const { data: signedData, error: signedError } = await supabase.storage
            .from("department_strategic_plan")
    ...
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load strategic plans.";
  }
}

document.addEventListener("DOMContentLoaded", loadStrategicPlans);
