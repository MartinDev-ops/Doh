...
async function loadAnnualPerformancePlans() {
  ...
  try {
    const { data, error } = await supabase
      .from("annual_performance_plan") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No annual performance plans available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: annual_performance_plan
          const { data: signedData, error: signedError } = await supabase.storage
            .from("annual_performance_plan")
    ...
    spinner.style.display = "none";
    console.error("Error fetching annual performance plans:", err);
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load annual performance plans.";
  }
}

document.addEventListener("DOMContentLoaded", loadAnnualPerformancePlans);
