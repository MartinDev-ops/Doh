...
async function loadArchivedTenders() {
  ...
  try {
    const { data, error } = await supabase
      .from("archived_tenders") // ✅ table name
      .select("*")
    ...
    if (!data || data.length === 0) {
      ...
      noNewsMsg.textContent = "No archived tenders available.";
      return;
    }
    ...
    data.forEach(doc => {
      ...
      card.onclick = async () => {
        try {
          ...
          // ✅ bucket name: archived_tenders
          const { data: signedData, error: signedError } = await supabase.storage
            .from("archived_tenders")
    ...
    spinner.style.display = "none";
    console.error("Error fetching archived tenders:", err);
    noNewsMsg.style.display = "block";
    noNewsMsg.textContent = "Failed to load archived tenders.";
  }
}

document.addEventListener("DOMContentLoaded", loadArchivedTenders);
